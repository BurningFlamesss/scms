...
const campuses = [
	{
		...
		name: school.nameEn,
		role: "Main school",
		address: `${school.address.line1}, ${school.address.line2}`,
		image: photos.campusModern.src,
	},
	{
		id: "canon",
		name: "Canon",
		role: "Affiliated building",
		address: school.address.line2,
		image: photos.campusFacade.src,
	},
];
...
export function CampusShowcase({ compact = false }: { compact?: boolean }) {
	return (
		<section
			className={`campus-showcase ${compact ? "campus-showcase--compact" : ""}`}
			data-testid="campus-showcase"
		>
			<div className="u-container">
				...
				<div className="campus-showcase__headline">
					...
					<p>Two buildings, connected parts of the same institution.</p>
				</div>
			</div>
				...
				{campuses.map((campus, index) => (
					<article key={campus.id} className="campus-panel">
						...
						<div className="campus-panel__content">
							...
							<p>{campus.role}</p>
							<p data-testid={`campus-${campus.id}-address`}>
								{campus.address}
							</p>
						</div>
					</article>
			...
			{!compact ? (
				<div className="u-container campus-showcase__link">
					<ArrowLink to="/contact" testId="campus-contact-link">
						Plan a conversation with the school
					</ArrowLink>
				</div>
			) : null}
		</section>
