...
function DetailPanel({ facility, onClose }: { facility: Facility; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key !== 'Tab') return;
      const nodes = ref.current?.querySelectorAll<HTMLElement>('button, [href], [tabindex]:not([tabindex="-1"])');
      ...
      >
        <div className='flex items-start justify-between gap-4 border-b border-border p-8'>
          <div className='flex flex-col gap-2'>
            ...
            <h3 className='u-display text-display-m text-foreground'>{facility.name}</h3>
          </div>
          <button
            ref={closeRef}
            type='button'
        ...
        <div className='flex flex-col gap-8 p-8'>
          <p className='max-w-measure text-body-l text-muted-foreground'>{facility.note}</p>
          ...
          <dl className='flex flex-col'>
            <div className='flex justify-between gap-4 border-t border-border py-3'>
              ...
              <dd className='u-label tnum text-foreground'>{facility.capacity}</dd>
            </div>
          ...
          <ul className='flex flex-col'>
            {facility.specs.map((s) => (
              <li key={s} className='border-b border-border py-3 text-body-m text-foreground'>
                {s}
              </li>
            ))}
          </ul>
          ...
          <div className='grid gap-4 sm:grid-cols-2'>
            {facility.images.map((im) => (
              <div key={im.src} className='overflow-hidden rounded-card border border-border'>
      ...
      {facilities.map((f, i) => (
        <li key={f.id} className='border-t border-border last:border-b'>
          <button
            ...
            className={cn(
              'flex w-full items-baseline gap-4 py-4 text-left transition-colors duration-micro ease-state',
              active === f.id ? 'text-foreground' : 'text-muted-foreground',
            )}
          >
            <span className='u-label tnum w-6 shrink-0 text-accent'>{String(i + 1).padStart(2, '0')}</span>
  ...
  return (
    <div className='flex flex-col gap-8' data-testid='campus-plan'>
      <div className='flex flex-col gap-2'>
        ...
        <WidgetCaption>Select a block, or use the list, to open its detail.</WidgetCaption>
      </div>
        ...
        {desktop && !reduced ? (
          <div className='relative lg:col-span-7'>
            <svg
              ...
              aria-hidden='true'
            >
              <rect x='0' y='0' width={VW} height={VH} fill='var(--c-gray-50)' />
              <text x='555' y='215' textAnchor='middle' fill='var(--c-gray-500)' fontSize='16' letterSpacing='2' fontFamily="'IBM Plex Mono', monospace">
                ASSEMBLY GROUND
              </text>
              <line x1='0' y1='560' x2={VW} y2='560' stroke='var(--c-gray-300)' strokeWidth='1' />
              <text x='24' y='584' fill='var(--c-gray-500)' fontSize='14' letterSpacing='2' fontFamily="'IBM Plex Mono', monospace">
              ...
              {facilities.map((f) => {
                ...
                return (
                  <g
                    key={f.id}
                    style={{
                      transform: on ? `translateY(-${motionScale.planLift}px)` : 'none',
            ...
            <AnimatePresence>
              {activeFacility && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  transition={{ duration: duration.micro / 1000, ease: easing.state }}
                  ...
                  style={{
                    left: `min(calc(${(activeFacility.block.cx / VW) * 100}% - 120px), calc(100% - 248px))`,
                ...
                >
                  ...
                  <div className='flex flex-col gap-1 p-4'>
                    ...
                    <p className='text-body-m text-foreground'>{activeFacility.name}</p>
                    <p className='text-body-s text-muted-foreground'>{activeFacility.note}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <AnimatePresence>{open ? <DetailPanel facility={open} onClose={close} /> : null}</AnimatePresence>
    </div>
  );
}