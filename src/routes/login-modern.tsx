  ...
  return (
    <section className="auth-page" aria-labelledby="login-title">
      ...
      <div className="auth-panel">
        <Link to="/" className="auth-back" data-testid="login-back-home-link" style={{color: 'var(--c-black)'}}>
          <ArrowLeft aria-hidden="true" style={{color: 'var(--c-red)'}} /> Public website
        </Link>
        <div className="auth-panel__inner">
          <span className="auth-kicker">SCHOOL MANAGEMENT SYSTEM</span>
          <h1 id="login-title">Welcome back.</h1>
          <p className="auth-intro" style={{color: 'var(--c-black)'}}>Enter the credentials issued by {config.organization.name} to continue.</p>
          ...
          <form onSubmit={submit} noValidate className="auth-form" data-testid="login-form">
            <div className={`form-control ${errors.email ? "form-control--error" : ""}`}>
              <Label htmlFor="login-email" style={{color: 'var(--c-black)'}}>Email</Label>
              <Input
                id="login-email"
                name="email"
                type="email"
            ...
            <div className={`form-control ${errors.password ? "form-control--error" : ""}`}>
              <div className="label-line">
                <Label htmlFor="login-password" style={{color: 'var(--c-black)'}}>Password</Label>
                <button type="button" onClick={() => setRecoveryOpen((value) => !value)} data-testid="forgot-password-button" style={{color: 'var(--c-yellow)', fontWeight: 500}}>Forgot password?</button>
              </div>
            ...
            <p className="demo-disclosure" style={{color: 'var(--c-yellow)'}}>Frontend demonstration — no credentials are transmitted or stored.</p>
          </form>

          <div className="auth-contact">
            <span style={{color: 'var(--c-black)'}}>Don't have access?</span>
            <Link to="/contact" data-testid="login-contact-school-link" style={{color: 'var(--c-red)', fontWeight: 500}}>Contact the school <span aria-hidden="true">→</span></Link>
          </div>
        </div>
      </div>
    </section>
