...
import { countdownLabel, eventsInTerm, eventsOnBs, nextOfKind, today } from '#/lib/calendar';
...
function Glyph({ kind }: { kind: EventKind }) {
  ...
  return (
    <svg width='9' height='9' viewBox='0 0 10 10' aria-hidden='true' className='shrink-0'>
      {g === 'square' ? <rect x='1' y='1' width='8' height='8' fill='currentColor' /> : null}
      {g === 'circle' ? <circle cx='5' cy='5' r='4' fill='currentColor' /> : null}
      {g === 'diamond' ? <path d='M5 0 L10 5 L5 10 L0 5 Z' fill='currentColor' /> : null}
      {g === 'triangle' ? <path d='M5 0 L10 9 L0 9 Z' fill='currentColor' /> : null}
      {g === 'bar' ? <rect x='0' y='3.5' width='10' height='3' fill='currentColor' /> : null}
    </svg>
  );
}
...
function RouteComponent() {
  ...
  const monthStartAd = bsToAd(year + '-' + String(month).padStart(2, '0') + '-01') ?? '';
  ...
  const moveFocus = (days: number, fromBs: string) => {
    ...
    window.setTimeout(() => {
      document.querySelector<HTMLElement>('[data-cell="' + nextBs + '"]')?.focus();
    }, 0);
  };
  ...
  const onCellKey = (e: React.KeyboardEvent, bs: string) => {
    const keys: Record<string, number> = { ArrowRight: 1, ArrowLeft: -1, ArrowDown: 7, ArrowUp: -7 };
          ...
          >
            {[
              ...
              <div key={c.label} className='flex flex-col gap-2 bg-paper p-6'>
                ...
                {c.ev ? (
                  <>
                    <p className='u-display text-display-m text-ink'>{c.ev.title}</p>
                    <p className='u-label tnum text-accent'>
                      {countdownLabel(c.ev.date.ad)} &middot; {formatBs(c.ev.date.bs)} BS &middot;{' '}
                      {formatAd(c.ev.date.ad)}
                    </p>
                  </>
                ) : (
                  <p className='text-body-m text-n-600'>Nothing further is scheduled this session.</p>
                )}
              </div>
            ))}
          </div>
      ...
      <Section testId='calendar-grid-section' className='pt-0'>
        <Container>
          <div className='flex flex-col gap-6'>
            ...
            <div className='flex flex-wrap items-end justify-between gap-6 border-y-hair border-n-200 py-6'>
              ...
              <div className='flex items-center gap-4'>
                ...
                <div className='min-w-[220px] text-center'>
                  <p className='u-display text-display-m text-ink' data-testid='calendar-month-label'>
                    {primary === 'bs'
                  ...
                  <p className='u-label text-n-600'>
                    {primary === 'bs'
                      ? BS_MONTHS_NE[month - 1] + ' ' + toNepaliDigits(year) + ' \u00b7 ' + formatAd(monthStartAd)
            ...
            <div className='flex flex-wrap items-center gap-2' data-testid='calendar-year-strip'>
              <span className='u-label mr-2 text-n-600'>Jump to month</span>
              {BS_MONTHS_EN.map((m, i) => (
                <button
                  ...
                  onClick={() => {
                    setMonth(i + 1);
                    setFocusBs(year + '-' + String(i + 1).padStart(2, '0') + '-01');
                  }}
                  aria-pressed={month === i + 1}
                  data-testid={'calendar-month-' + (i + 1)}
                  className={cn(
                    'u-label min-h-tap rounded-pill border-hair px-3',
                    month === i + 1
                      ? 'border-ink bg-ink text-paper'
                      : 'border-n-300 text-n-600 hover:border-ink hover:text-ink',
                  )}
                >
                  {m.slice(0, 3)}
                </button>
            ...
            <div
              role='grid'
              aria-label={'Academic calendar, ' + BS_MONTHS_EN[month - 1] + ' ' + year + ' Bikram Sambat'}
              ...
              {Array.from({ length: Math.ceil(cells.length / 7) }, (_, w) => (
                <div role='row' className='grid grid-cols-7' key={w}>
                  {cells.slice(w * 7, w * 7 + 7).map((c) => {
                    const evs = eventsOnBs(c.bs).filter((e) => visibleKinds.includes(e.kind));
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          c.weekday === 6 && 'bg-n-50',
                          isSelected && 'outline outline-rule -outline-offset-2 outline-accent',
                        )}
                      >
                        ...
                        {isToday ? <span className='u-label text-accent'>Today</span> : null}
            ...
            <AnimatePresence initial={false}>
              {selected ? (
                <motion.div
                  ...
                  transition={{ duration: (reduced ? 0 : duration.standard) / 1000, ease: easing.state }}
                  className='overflow-hidden'
                  data-testid='calendar-detail'
                >
                  <div className='border-b-rule border-accent bg-n-50 p-6'>
                    <p className='u-label text-accent'>
                      {formatBs(selected)} BS &middot; {formatBs(selected, { nepali: true })} &middot;{' '}
                      {formatAd(bsToAd(selected) ?? '')}
                    </p>
                    {selectedEvents.length === 0 ? (
                      <p className='mt-3 text-body-l text-ink'>
                        Nothing scheduled on this date. It is a normal teaching day.
                      </p>
                    ) : (
                      <ul className='mt-4 flex flex-col'>
                        {selectedEvents.map((e) => (
                          ...
                          >
                            <span className={cn('u-label inline-flex items-center gap-2', KIND_TEXT[e.kind])}>
                              <Glyph kind={e.kind} />
                              {eventKindMeta[e.kind].label}
                            </span>
                            <span className='min-w-0 flex-1'>
                              <span className='block text-body-l text-ink'>{e.title}</span>
                              {e.titleNe ? (
                                <span className='u-ne block text-body-m text-n-600'>{e.titleNe}</span>
                              ) : null}
                              {e.detail ? (
                                <span className='block text-body-s text-n-600'>{e.detail}</span>
                              ) : null}
                              {e.endDate ? (
                                <span className='u-label block text-n-600'>
                                  until {formatBs(e.endDate.bs)} BS &middot; {formatAd(e.endDate.ad)}
                                </span>
                              ) : null}
                            </span>
                            <Button
            ...
            <div className='mt-6 flex flex-wrap items-center gap-2' data-testid='calendar-legend'>
              <span className='u-label mr-2 text-n-600'>Show</span>
              {KINDS.map((k) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'u-label inline-flex min-h-tap items-center gap-2 rounded-pill border-hair px-4',
                      on ? 'border-ink text-ink' : 'border-n-300 text-n-500 line-through',
                    )}
                  >
          ...
          <ul className='flex flex-col'>
            {calendarEvents
              ...
              .map((e) => (
                ...
                >
                  <span className={cn('u-label inline-flex w-[152px] items-center gap-2', KIND_TEXT[e.kind])}>
                    <Glyph kind={e.kind} />
                    {eventKindMeta[e.kind].label}
                  </span>
                  <span className='min-w-0 flex-1 text-body-m text-ink'>{e.title}</span>
                  <span className='u-label tnum text-n-600'>{formatBs(e.date.bs)} BS</span>
                  <span className='u-label tnum w-[168px] text-right text-n-600'>{formatAd(e.date.ad)}</span>
                </li>
              ))}
          </ul>
        </Container>
